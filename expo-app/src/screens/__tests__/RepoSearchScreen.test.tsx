...
const searchMock = searchRepositories as jest.MockedFunction<typeof searchRepositories>;
...
describe('RepoSearchScreen', () => {
  ...
  it('shows the keyword handed over by the host app', async () => {
    const { getByText } = await render(<RepoSearchScreen keyword="expo-brownfield" />);
  ...
  it('reports the results back to the host app', async () => {
    ...
    await waitFor(() => expect(notifySearchSucceeded).toHaveBeenCalledWith('expo', results));
  ...
  it('shows the failure and reports it to the host app', async () => {
    ...
    await waitFor(() => expect(getByText('API rate limit exceeded')).toBeTruthy());
    expect(notifySearchFailed).toHaveBeenCalledWith('expo', 'API rate limit exceeded');
